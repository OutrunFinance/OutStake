...
像铭文一样发行 ERC20 Token, 用户通过付费 mint token, 在 mint 的过程中 mint 的费用将会和合约中预留的一部分 Token 组成 LP 在 Outswap 上添加流动性，并设置自定义的交易手续费。LP 将锁定一段时间，到期后用户可以取出自己的 mint 费与预留 token 组成的 LP 以及产生的原生收益，而项目团队募集的资金是就是交易手续费。
