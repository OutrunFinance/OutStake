...
Issuing ERC20 tokens like inscription, users can mint tokens by paying a fee. During the minting process, the fee along with a portion of tokens reserved in the contract form a trading pair to provide liquidity on Outswap.LP will be locked for a period, and users will receive YieldTokens by locking their minting fees (staking ETH or USDB). Upon the expiration of the lock-up period, users can withdraw their LP and remove liquidity, while the funds raised by the project team are the trade fees from trading pairs during the LP lock-up period.Given the core requirements mentioned above, we also offer customized minter and deployer contracts for project teams to enhance the flexibility of the LaunchPad.

This model fosters fairness and is more investor-friendly. Investors effectively acquire the project's tokens at no cost. Furthermore, it prevents project teams from losing interest in product development after raising substantial funds, as often seen in traditional IDOs. To raise further funds, project teams must continuously iterate on products during the LP lock-up period, encouraging users to trade their tokens. This facilitates sustained growth in cash flow.
