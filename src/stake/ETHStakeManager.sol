...
contract ETHStakeManager is IETHStakeManager, AccessControl {
    ...
    address public yieldPosition;
    uint256 public minIntervalTime;

    mapping(uint256 account => uint256 positionId) public positions;    // positionId = NFT tokenId
     ...
     * @param _yieldPosition - Address of the YieldPosition
    ...
    constructor(
        ...
        yieldPosition =_yieldPosition;

        _grantRole(DEFAULT_ADMIN_ROLE, _admin);
    }

    /**
     * 用户stake BETH，指定一个锁定到期时间deadLine，锁定到期前不可unstake，铸造相同数量的PETH和收益NFT
     *
     * @dev Allows user to deposit BETH, then mints PETH and yieldPosition NFT for the user.
    ...
    /**
     * 用户unstake,首先判断锁定时间是否已过期，否则不能unstake。t
     * 最后将PETH销毁，仅将BETH取出来，只有作为收益证明的NFT才能将质押收益取出来。
     *
